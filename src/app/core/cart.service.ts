...
export class CartService {
  ...
  addItem(item: OfferItem) {
    ...
    if (existingItem) {
      this.cartItems.set(currentCart.map(ci => 
  ...
  updateNotes(itemId: number, notes: string) {
    this.cartItems.update(cart => cart.map(ci => 
      ci.item.item_id === itemId ? { ...ci, notes } : ci
    ));
  }
  ...
  getQuantityDifferences(): { item_id: number; quantityDiff: number }[] {
    ...
    console.log('Current cart:', currentCart.map(c => ({ id: c.item.item_id, name: c.item.item_name, qty: c.quantity })));
    console.log('Placed order:', placed?.items.map(c => ({ id: c.item.item_id, name: c.item.item_name, qty: c.quantity })));
    ...
    currentCart.forEach(currentItem => {
      ...
      if (placedItem) {
        ...
        console.log(`Item ${currentItem.item.item_name}: current=${currentItem.quantity}, placed=${placedItem.quantity}, diff=${diff}`);
        if (diff !== 0) {
