...
export class LocationPickerDialog implements AfterViewInit, OnDestroy {
  ...
  private readonly data = inject<{ coords?: google.maps.LatLngLiteral; label?: string }>(MAT_DIALOG_DATA);
  private readonly mapsLoader = inject(GoogleMapsLoaderService);
  private readonly locationLookup = inject(LocationLookupService);

  private readonly searchModel = signal({ search: '' });
  ...
  // Set right before a programmatic write to searchModel (e.g. from a resolved
  // place/pin) so the search-value effect below doesn't treat it as user typing
  // and immediately invalidate the verification it just set.
  ...
  constructor() {
    ...
    this.subscriptions.add(
      this.searchInput$.pipe(debounceTime(300), distinctUntilChanged()).subscribe((value) => this.fetchSuggestions(value)),
    );

    effect(() => {
      const value = this.searchModel().search;
      if (this.suppressNextSearchChange) {
