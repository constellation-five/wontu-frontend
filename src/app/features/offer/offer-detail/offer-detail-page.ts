...
@Component({
  ...
  imports: [
    ...
    PageHeaderComponent, CustomCardComponent, CardItemComponent
  ],
...
export class OfferDetailPage implements OnInit {
  ...
  decreaseQuantity(cartItem: CartItem) {
    if (cartItem.quantity === 1) {
      ...
      const dialogData: DialogData = {
        ...
        buttons: [
          { label: 'Cancel', type: 'outlined', action: 'cancel', borderColor: '#B3B8C7', textColor: '#52555D' },
          { label: 'Delete', icon: 'delete', type: 'filled', action: 'delete', bgColor: '#D32F2F', textColor: '#FFFFFF' }
        ]
      };
  ...
  increaseQuantityFromCard(itemId: number) {
    this.cartItems.update(cart => cart.map(ci => 
      ci.item.item_id === itemId ? { ...ci, quantity: ci.quantity + 1 } : ci
    ));
    // Sync with cart service
    this.cartService.setCartItems(this.cartItems());
  }
